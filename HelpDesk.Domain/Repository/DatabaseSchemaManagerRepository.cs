...
namespace HelpDesk.Domain.Repository
{
    ...
    public class DatabaseSchemaManagerRepository : IDatabaseSchemaManagerRepository
    {
        ...
        public DatabaseSchemaManagerRepository(IApplicationSettingsRepository appService)
        {
            AppSettings = appService;
        }

        public IApplicationSettingsRepository AppSettings { get; set; }

        public void EnsureSchemaVersion()
        {
            try
            {
                var currentVersionSetting = AppSettings.GetAllSettings().SingleOrDefault(s => s.SettingName == "Version");
        ...
        public void ExecuteUpgrade(string script)
        {
            ...
            {
                var cString = (ctx.Database.Connection as EntityConnection).StoreConnection.ConnectionString;//get connection string from entity framework context
