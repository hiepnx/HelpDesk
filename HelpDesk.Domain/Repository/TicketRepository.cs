...
namespace HelpDesk.Domain.Repository
{
    ...
    public class TicketRepository : ITicketRepository
    {
        ...
        public IPagination<Ticket> ListTickets(int pageIndex, int pageSize, List<TicketListSortColumn> sortColumns, List<TicketListFilterColumn> filterColumns, bool includeComments)
        {
            ...
            ObjectQuery<Ticket> tq = (includeComments) ? ctx.ObjectContext.CreateObjectSet<Ticket>().Include("TicketComments") : ctx.ObjectContext.CreateObjectSet<Ticket>();
        ...
        public IEnumerable<Ticket> ListTickets(SortedList<int, int> orderedTicketList, bool includeComments)
        {
            ...
            ObjectQuery<Ticket> tickets = (includeComments) ? ctx.ObjectContext.CreateObjectSet<Ticket>().Include("TicketComments") : ctx.ObjectContext.CreateObjectSet<Ticket>();
        ...
        {
            ctx.TicketAttachments.Remove(attachment);
            if (commit)
            {
        ...
        {
            ctx.TicketAttachments.Add(attachment);
            if (commit)
            {
                ctx.SaveChanges();
                if (attachment.Ticket != null)//new tickets page can add and remove attachments before the ticket physically exists in the DB
        ...
        public bool CleanUpDerelictAttachments(int hoursOld)
        {
            ...
            {
                ctx.TicketAttachments.Remove(att);
            }
            ctx.SaveChanges();
            return true;
        }
        ...
        public Dictionary<string, object> GetTicketChanges(Ticket modifiedTicket)
        {
            ...
            var ose = ctx.ObjectContext.ObjectStateManager.GetObjectStateEntry(modifiedTicket);
