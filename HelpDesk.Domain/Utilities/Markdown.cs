...
namespace HelpDesk.Domain.Utilities
{
    public class Markdown
    {
        ...
        /// <summary>
        /// Transforms the specified text to HTML.
        /// </summary>
        /// <param name="text">The text to transform.</param>
        /// <param name="sanitize">if set to <c>true</c> sanitize (remove) all HTML tags from the input string.</param>
        /// <returns></returns>
        /// <remarks>
        ...
        /// </remarks>
        public string Transform(string text, bool sanitize)
        {
            if (sanitize)
            {
                text = Sanitize(text);
            }
            if (string.IsNullOrEmpty(text))//SMR - Fixed null ref bug when text is not supplied
            {
                return text;
            }
        ...
        #region SMR - sanitizer
        private Regex _tags = new Regex("<[^>]*(>|$)", RegexOptions.Singleline | RegexOptions.ExplicitCapture | RegexOptions.Compiled);


        //SMR - The below is part of the original code, but is not being used here... leaving the code here for informational reference

        //        private Regex _whitelist = new Regex(@"
        ...
        //        private Regex _whitelist_a = new Regex(@"
        //            ^<a\s
        ...
        //        private Regex _whitelist_img = new Regex(@"
        //            ^<img\s
        //            src=""https?://[-a-z0-9+&@#/%?=~_|!:,.;\(\)]+""
        //            (\swidth=""\d{1,3}"")?
        //            (\sheight=""\d{1,3}"")?
        ...
        public string Sanitize(string html)
        {
            ...
            for (int i = tags.Count - 1; i > -1; i--)
            {
                ...
                //SMR - removing whitelist checks, leaving here for informational reference
