...
SET 
	...
	SettingDescription = 'Setting this to "true" will cause the system to delete stored profile settings for ALL users.\n\nThis is sometimes useful after upgrading HelpDesk to a new version to reset user''s TicketCenter List Settings. The system will auto-regenerate a default set of profile values.'
WHERE
...
BEGIN
	INSERT Settings(SettingName,SettingValue,DefaultValue,SettingType,SettingDescription)
	VALUES('EmailResendDelayMinutes','5','5','IntString','This setting detemines how long HelpDesk will wait if there is an error when trying to send an email notification. Each time the notification failes, the wiat time will be the number of attempts times the value of this setting: Example, if the setting is 5 and message fails the third attempt, it will wait 15 minutes before trying again.')
END
GO
...
BEGIN
	INSERT Settings(SettingName,SettingValue,DefaultValue,SettingType,SettingDescription)
	VALUES('SiteRootUrlForEmail','http://localhost:2534','http://localhost:2534','SimpleString','The root URL of the web site; will be used in email notifications to create the fully qualified link URLs. Without a valid setting, users clicking links in their notification email will not be directed to the HelpDesk site.')
END
GO
...
BEGIN
	INSERT Settings(SettingName,SettingValue,DefaultValue,SettingType,SettingDescription)
	VALUES('FromEmailDisplayName','HelpDesk','HelpDesk','SimpleString','The "friendly name" to use in the "TO" field of email notifiacitons.')
END
GO
...
BEGIN
	INSERT Settings(SettingName,SettingValue,DefaultValue,SettingType,SettingDescription)
	VALUES('FromEmailAddress','HelpDesk@nowhere.com','HelpDesk@nowhere.com','SimpleString','The email address to use in the "TO" field of email notifiacitons.\n\nUsually this is not a real person''s email address, just a made up address at your organization.')
END
GO
...
BEGIN
	...
	INSERT Settings(SettingName,SettingValue,DefaultValue,SettingType,SettingDescription)
	VALUES('Version','2.0.0','','SimpleString','The version of the HelpDesk database. CHANGE AT YOUR OWN RISK!')
END
ELSE
BEGIN
	UPDATE Settings 
